...
where:
  ...
  ExerciseExtra = (MET - 1) × weight × hours
    対象 session: 自社 HIIT (MET 8.0) + HC `ExerciseSessionRecord` (type 別 MET 表)
...
HIIT セッションを受信したタイミングで HC に書き戻す (`HealthDataSource.writeHiitSession`):

- `ExerciseSessionRecord(type = HIGH_INTENSITY_INTERVAL_TRAINING)` (start / end)
- `ActiveCaloriesBurnedRecord` (HIIT MET ベースで `(8 - 1) × weight × hours`)
- `clientRecordId = "ap-{sessionId}"` で再受信時の冪等性を確保

権限は `WRITE_EXERCISE` / `WRITE_ACTIVE_CALORIES_BURNED`。他アプリ (Google Fit /
MyFitnessPal 等) が HC を読んだときも本アプリの値が見える。

HC には「他アプリの書き込みを禁止する」primitive が無い (alpha 11 時点)。代わりに
**読む側**で自社が信頼する dataOrigin / type 構成 (BMR aggregate + HC ExerciseSession 型 + 自社 HIIT)
だけで TDEE を組むことで、watch / Fitbit が書く inflated total を deficit 計算に
混ぜない運用にしている。完全に絶ちたい場合はユーザが HC 設定で対応アプリの書き込み
権限を手動 revoke する。
