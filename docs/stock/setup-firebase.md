...
1. **phone 実機に sideload**: `bash scripts/sideload.sh phone` (最新 release の署名済 APK を `gh release download` + `adb install`。詳細は script ヘッダ。watch も同じ script に `watch` を渡せばよく、ウォッチ側のワンタイム設定は `docs/stock/setup-watch.md`)
